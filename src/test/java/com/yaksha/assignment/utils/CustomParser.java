...
public class CustomParser {
	...
	public static boolean checkMethodAnnotation(String methodName, Class<?>[] paramTypes, String expectedValue) {
		try {
			Method method = com.yaksha.assignment.controller.UserController.class.getMethod(methodName, paramTypes);
	...
	public static boolean checkMethodAnnotationWithValueAgain(String filePath, String methodName,
			String annotationValue) throws IOException {
		System.out.println("Checking method-level annotation with value in file: " + filePath);
		...
		boolean hasMethodAnnotationWithValue = classDeclaration.get().getMethods().stream()
				.anyMatch(method -> method.getAnnotationByName("GetMapping").isPresent()
						&& method.getAnnotationByName("GetMapping").get().toString().contains(annotationValue));

		if (!hasMethodAnnotationWithValue) {
			System.out
					.println("Error: No method is annotated with the required value in the " + methodName + " method.");
			return false;
		}

		System.out.println("Method " + methodName + " has the correct annotation value.");
		return true;
	}

	/**
	 * Checks if a specific method exists in the class.
	 *
	 * @param filePath   The full path to the Java class file.
	 * @param methodName The name of the method to check.
	 ...
	 */
	public static boolean checkMethodExists(String filePath, String methodName) throws IOException {
		// Load class content
		String classContent = loadClassContent(filePath);
		if (classContent == null) {
			System.out.println("Error: Failed to load class content from file: " + filePath);
		...
		boolean methodExists = compilationUnit.getTypes().stream().flatMap(type -> type.getMethods().stream()) // Get
																												// methods
																												// from
																												// the
																												// class
				.anyMatch(method -> method.getNameAsString().equals(methodName)); // Check if the method name matches

		if (methodExists) {
			System.out.println("Method '" + methodName + "' found in the class.");
		} else {
	...
	/**
	 * Checks if a specified method is called inside any method of a specific class.
	 *
	 * @param filePath   The full path to the Java class file.
	 * @param className  The name of the class to check for method calls.
	 * @param methodName The name of the method to check if it's called in the class
	 *                   methods.
	 * @return true if the specified method is called inside any method of the
	 *         class, false otherwise.
	 * @throws IOException If an error occurs while reading the class file.
	 */
	public static boolean checkMethodCalledInMethods(String filePath, String className, String methodName)
			throws IOException {
		// Load class content
		String classContent = loadClassContent(filePath);
		if (classContent == null) {
		...
		// Get the specified class from the file
		Optional<ClassOrInterfaceDeclaration> classDeclaration = compilationUnit.getClassByName(className);

		if (classDeclaration.isEmpty()) {
			System.out.println("Error: The class '" + className + "' not found in the file.");
			return false;
		}

		// Check if the specified method is called in any method of the class
		boolean usesMethod = classDeclaration.get().getMethods().stream()
				.anyMatch(method -> method.getBody().isPresent() && method.getBody().get().findAll(MethodCallExpr.class)
						.stream().anyMatch(call -> call.getNameAsString().equals(methodName)));

		if (usesMethod) {
			System.out.println("Success: The '" + methodName
					+ "' method is called inside at least one method in the class '" + className + "'.");
		} else {
			System.out.println("Error: The '" + methodName + "' method is not called inside any method in the class '"
					+ className + "'.");
		}
